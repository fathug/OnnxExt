...
namespace OnnxExtBatch
{
    class Program
    {
            ...
            using (var session = new InferenceSession(modelPath, gpuSessionOptoins))
            {
                ...
                using (var results = session.Run(inputs))
                {
                    ...
                    if (outputTensor != null)
                    {
                        ...
                        {
                            DrawBoundingBoxes(objectResultsNms[i], imagePaths[i]);
                        }
                    }
                    else
                    {
        ...
        static DenseTensor<float> PreprocessImages(List<string> imagePaths, int batchSize)
        {
            ...
            // 在堆上分配内存
            float[] inputData = new float[batchSize * pixelNum * 3];

            for (int b = 0; b < batchSize; b++)
            {
                // 如果图片数量不足 BatchSize，则使用最后一张图片填充
                string imagePath = b < imagePaths.Count ? imagePaths[b] : imagePaths.LastOrDefault();
                        ...
                        unsafe
                        {
                            ...
                            for (int y = 0; y < InputHeight; y++)
                            {
                                for (int x = 0; x < InputWidth; x++)
                                {
                                    ...
                                    {
                                        pInputData[b * pixelNum * 3 + index + pixelNum * 0] = ptr[ptrIndex + 2] / 255f; // R
                                        pInputData[b * pixelNum * 3 + index + pixelNum * 1] = ptr[ptrIndex + 1] / 255f; // G
                                        pInputData[b * pixelNum * 3 + index + pixelNum * 2] = ptr[ptrIndex + 0] / 255f; // B
                                    }
                                }
                            }
                        }
            ...
            return new DenseTensor<float>(inputData, new int[] { batchSize, 3, InputHeight, InputWidth });
        ...
        static List<List<ObjectResult>> ProcessOutput(DenseTensor<float> outputTensor)
        {
            ...
            for (int b = 0; b < BatchSize; b++)
            {
                ...
                {
                    int idxPerBatch = (outputShape[1] * outputShape[2]) * b;

                    int idxConfidence = idxPerBatch + (lengthD4 * i + 4); // 每个检测结果中的置信度索引
                    ...
                    if (confidence > Confidence)
                    {
                        ...
                        int idxEdn =   idxConfidence + numClasses;
        ...
        static List<List<ObjectResult>> NonMaxSuppressionBatch(List<List<ObjectResult>> boxesBatch, float iouThreshold)
        {
            List<List<ObjectResult>> nmsBatchedResults = new List<List<ObjectResult>>();
            foreach (var boxes in boxesBatch)
            {
                nmsBatchedResults.Add(NonMaxSuppression(boxes, iouThreshold));
            }
