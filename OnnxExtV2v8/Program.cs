...
/// </summary>
/// <remarks>
/// 2025-03-12 新建。支持yolov8模型推理，与v5的区别在输出张量处理的部分。
/// </remarks>
...
namespace OnnxExt
{
    class Program
    {
        ...
        {
            // 文件路径
            string modelPath = Path.Combine(ConfigDirectory, "model/yolov8n.onnx");
            string imagePath = Path.Combine(ConfigDirectory, "image3/1.jpg") ;
        ...
        // 处理模型输出结果的函数
        static List<ObjectResult> ProcessOutput(DenseTensor<float> outputTensor0, string imagePath)
        {
            // 获取输出张量的形状
            int[] outputShape0 = outputTensor0.Dimensions.ToArray();

            Console.WriteLine($"本次输出张量形状: [{string.Join(",", outputShape0)}]");

            // 输出张量做转置
            var outputShape = new int[] { outputShape0[0], outputShape0[2], outputShape0[1] };
            var outputTensor = new DenseTensor<float>(outputShape);
            for (int i = 0; i < outputShape0[0]; i++)
            {
                for (int j = 0; j < outputShape0[1]; j++)
                {
                    for (int k = 0; k < outputShape0[2]; k++)
                    {
                        outputTensor[i, k, j] = outputTensor0[i, j, k];
                    }
                }
            }
            ...
            int numClasses = outputShape[2] - 4;
            ...
            {
                int idxStart = lengthD4 * i + 4;
                int idxEdn = lengthD4 * (i + 1);
                ...
                for (int j = idxStart; j < idxEdn; j++)
                {
                    ...
                    if (tmpClasaScore > maxClassScore)
                    {
                        ...
                        maxClassId = j - idxStart;//这里可能有问题
                    }
                }

                if (maxClassScore > Confidence)
                {
                    float x = outputTensor.GetValue(idxStart - 4);
                    float y = outputTensor.GetValue(idxStart - 3);
                    float w = outputTensor.GetValue(idxStart - 2);
                    float h = outputTensor.GetValue(idxStart - 1);
